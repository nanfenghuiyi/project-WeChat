...
Page({
  ...
  loadMore(){
    var data = this.data.playdata;
    data=JSON.parse(data)
    console.log(data)
    this.playdata=data
    var singer = data.singer
    var id=''
    if (!singer[0].name && !data.media_mid){
      singer = data.singer
      id=data.id
      // console.log(singer)
      // console.log(id)
    ...
    var songurl = `https://v1.itooi.cn/tencent/url?id=${id}&quality=128`
    var coverimg = `https://v1.itooi.cn/tencent/pic?id=${id}`
    this.setData({
      playdata:data,
      audiosrc: songurl,
      // coverimg: coverimg,
  ...
  onLoad: function (options) {
    ...
    //保存data中playdata
    this.setData({
      playdata: decodeURIComponent(options.data)
    });
    this.loadMore()
  },
  /**
  ...
  onShow: function () {
    //音频的数据链接，用于直接播放。
    innerAudioContext.src = this.data.playdata.url
    this.Initialization();
    this.loadaudio();
  },
